...
function Finance() {
  const savedList = JSON.parse(localStorage.getItem("ListTable"));
  const [tabela, setTabela] = useState(savedList ?? []);
  ...
  const handleSaveTable = (table) => {
    localStorage.setItem("ListTable", JSON.stringify(table));
    setTabela(table);
  };

  const handleAddTable = (value) => {
    const item = [...tabela, value];
    handleSaveTable(item);
  };

  const handleRemove = (id) => {
    if (window.confirm("Deseja deletar este item?")) {
      const tableRemoved = tabela.filter((item) => item.id !== id);
      handleSaveTable(tableRemoved);
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    const newValue = {
      id: Math.floor(Math.random() * 1000 * 1000 * 1000 * 1000 * 1000 * 1000),
      title,
      desc,
      value: Number(price),
    };
              ...
              {tabela.map((item) => (
                ...
                >
                  ...
                  <div className="flex flex-col items-end">
                    ...
                    <button
                      className="hover:bg-gray-300 p-1 rounded transition-all"
                      title="Deletar item"
